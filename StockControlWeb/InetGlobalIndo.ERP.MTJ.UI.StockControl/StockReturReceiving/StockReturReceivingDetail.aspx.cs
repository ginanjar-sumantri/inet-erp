...
{
    public partial class StockReturReceivingDetail : StockReturReceivingBase
    {
        ...
        private string _reportPath1 = "StockReturReceiving/JournalEntryPrintPreview.rdlc";
        private string _reportPath2 = "StockReturReceiving/JournalEntryPrintPreviewHomeCurr.rdlc";
        ...
        private double RowCount()
        {
            ...
            _result = this._stcReturBL.RowsCountSTCReturRRDt(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        {
            _stcReturBL = new STCReturBL();
            STCReturRRHd _stcReturHd = this._stcReturBL.GetSingleSTCReturRRHd(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));

            this.TransNoTextBox.Text = _stcReturHd.TransNmbr;
            this.FileNoTextBox.Text = _stcReturHd.FileNmbr;
            this.TransDateTextBox.Text = DateFormMapper.GetValue(_stcReturHd.TransDate);
            this.StatusLabel.Text = STCReturDataMapper.GetStatusText(_stcReturHd.Status);
            this.CustCodeTextBox.Text = this._customerBL.GetNameByCode(_stcReturHd.CustCode);
            this.ReqReturTextBox.Text = this._reqSalesReturBL.GetFileNmbrMKTReqReturHd(_stcReturHd.ReqReturNo);
            this.RemarkTextBox.Text = _stcReturHd.Remark;
            this.WarehouseCodeTextBox.Text = _warehouseBL.GetWarehouseNameByCode(_stcReturHd.WrhsCode);

            if (_stcReturHd.WrhsFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.NoSubled))
            {
                this.WrhsSubledTextBox.Text = "";
            }
            else
            {
                if (_stcReturHd.WrhsFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Customer))
                {
                    this.WrhsSubledTextBox.Text = this._customerBL.GetNameByCode(_stcReturHd.WrhsSubLed.ToString());
                }
                else if (_stcReturHd.WrhsFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Supplier))
                {
                    this.WrhsSubledTextBox.Text = this._supplierBL.GetSuppNameByCode(_stcReturHd.WrhsSubLed.ToString());
                }
            }

            this.StatusHiddenField.Value = _stcReturHd.Status.ToString();
            ...
            {
                this.ListRepeater.DataSource = this._stcReturBL.GetListSTCReturRRDt(_prmCurrentPage, _maxrow, this.TransNoTextBox.Text.Trim());
            }
            this.ListRepeater.DataBind();

            this.AllCheckBox.Checked = this.IsCheckedAll();

            if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                this.AddButton.Visible = false;
                this.DeleteButton.Visible = false;
                this.EditButton.Visible = false;
        ...
        {
            if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
        ...
        protected void ActionButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
                string _result = this._stcReturBL.GetAppr(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                string _result = this._stcReturBL.ApproveSTCReturRRHd(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
                string _result = this._stcReturBL.PostingSTCReturRRHd(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == STCReturDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                ...
                //string _result = this._stcReturBL.Unposting(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _result = this._stcReturBL.DeleteMultiSTCReturRRDt(this.TransNoTextBox.Text, _tempSplit);
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                STCReturRRDt _temp = (STCReturRRDt)e.Item.DataItem;
                //string _code = _temp.TransNmbr.ToString();
                string _code1 = _temp.ProductCode;
                string _code2 = _temp.LocationCode;
                string _code = _code1 + "|" + _code2;
        ...
        protected void PreviewButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ReportDataSource _reportDataSource1 = this._reportStock.StockReceivingReturPrintPreview(this.TransNoTextBox.Text);

            this.ReportViewer1.LocalReport.DataSources.Clear();
            this.ReportViewer1.LocalReport.DataSources.Add(_reportDataSource1);

            //string _path = _reportListBL.GetPathPrintPreview(AppModule.GetValue(TransactionType.StockReceivingRetur), _userBL.GetCompanyId(HttpContext.Current.User.Identity.Name));
            string _path = "StockReturReceiving/StockReceivingReturPrintPreviewWithoutProductCodeWithoutCompanyHeader.rdlc";
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            string _result = this._stcReturBL.UnpostingSTCReturRRHd(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

            //this.Label.Text = _result;

            if (_result == "Unposting Success")
            {
                bool _result1 = _unpostingActivitiesBL.InsertUnpostingActivities(AppModule.GetValue(TransactionType.StockRejectIn), this.TransNoTextBox.Text, this.FileNoTextBox.Text, HttpContext.Current.User.Identity.Name, this.ReasonTextBox.Text, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.UnPosting)));
                if (_result1 == true)
                    this.WarningLabel.Text = _result;
