...
{
    public partial class StockReturReceivingDetailView : StockReturReceivingBase
    {
        ...
        {
            STCReturRRDt _stcReturRRDt = this._stcReturBL.GetSingleSTCReturRRDt(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), Rijndael.Decrypt(this._nvcExtractor.GetValue(this._productKey), ApplicationConfig.EncryptionKey), Rijndael.Decrypt(this._nvcExtractor.GetValue(this._locationKey), ApplicationConfig.EncryptionKey));

            this.ProductCodeTextBox.Text = _productBL.GetProductNameByCode(_stcReturRRDt.ProductCode);
            this.LocationCodeTextBox.Text = _wrhsBL.GetWarehouseLocationNameByCode(_stcReturRRDt.LocationCode);
            this.QtyTextBox.Text = _stcReturRRDt.Qty.ToString("#,##0.##");
            this.UnitCodeTextBox.Text = _unitBL.GetUnitNameByCode(_stcReturRRDt.Unit);
            this.RemarkTextBox.Text = _stcReturRRDt.Remark;
        }
