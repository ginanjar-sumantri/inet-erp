...
{
    public partial class Convertion : ConversionBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._convBL.RowsCountConvertion(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        //{
        //    double q = this._convBL.RowsCountConvertion(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._convBL.GetListConvertion(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                MsConvertion _temp = (MsConvertion)e.Item.DataItem;
                string _unitCode = _temp.UnitCode.ToString();
                string _unitConvert = _temp.UnitConvert.ToString();
                string _join = _unitCode + "&" + _unitConvert;

                if (this.TempHidden.Value == "")
                {
                    this.TempHidden.Value = _join;
                }
                else
                {
                    this.TempHidden.Value += "," + _join;
                ...
                _listCheckbox.Attributes.Add("OnClick", "CheckBox_Click(" + this.CheckHidden.ClientID + ", " + _listCheckbox.ClientID + ", '" + _join + "', '" + _awal + "', '" + _akhir + "', '" + _cbox + "')");
                _listCheckbox.Checked = this.IsChecked(_join);

                ImageButton _viewButton = (ImageButton)e.Item.FindControl("ViewButton");
                _viewButton.PostBackUrl = this._viewPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_unitConvert, ApplicationConfig.EncryptionKey)) + "&" + this._unitKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_unitCode, ApplicationConfig.EncryptionKey));
                ...
                _editButton.PostBackUrl = this._editPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_unitConvert, ApplicationConfig.EncryptionKey)) + "&" + this._unitKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_unitCode, ApplicationConfig.EncryptionKey));
                ...
                _unitCodeLiteral.Text = HttpUtility.HtmlEncode(_temp.UnitCodeName);

                Literal _unitConvertLiteral = (Literal)e.Item.FindControl("UnitConvertLiteral");
                _unitConvertLiteral.Text = HttpUtility.HtmlEncode(_temp.UnitConvertName);

                Literal _rate = (Literal)e.Item.FindControl("RateLiteral");
                _rate.Text = HttpUtility.HtmlEncode(_temp.Rate.ToString("#,##0.##"));
                if (_temp.Rate < 1 && _temp.Rate > 0)
                {
                    _rate.Text = "0" + _rate.Text;
                }
            }
        }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _result = this._convBL.DeleteMultiConvertion(_tempSplit);
