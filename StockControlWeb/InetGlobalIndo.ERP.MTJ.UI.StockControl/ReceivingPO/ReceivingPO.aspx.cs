...
{
    public partial class ReceivingPO : ReceivingPOBase
    {
        ...
        #region code_by_oyaman
            ...
            {
                if (this.sortField.Value == "Supplier")
                {
                    if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                        this.ascDesc.Value = "true";
                    else
        ...
        private double RowCount()
        {
            ...
            _result = this._receivingPOBL.RowsCountSTCReceiveHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._receivingPOBL.GetListSTCReceiveHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, this.sortField.Value, Convert.ToBoolean(this.ascDesc.Value));
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                STCReceiveHd _temp = (STCReceiveHd)e.Item.DataItem;
                ...
                if (_temp.Status != ReceivingPODataMapper.GetStatus(TransStatus.Posted))
                {
                ...
                _status.Text = HttpUtility.HtmlEncode(ReceivingPODataMapper.GetStatusText(_temp.Status));

                //Literal _warehouse = (Literal)e.Item.FindControl("WarehouseLiteral");
                //_warehouse.Text = HttpUtility.HtmlEncode(_warehouseBL.GetWarehouseNameByCode(_temp.WrhsCode));

                //Literal _warehouseSubled = (Literal)e.Item.FindControl("WrhsSubledLiteral");
                //if (_temp.WrhsFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Customer))
                //{
                //    _warehouseSubled.Text = HttpUtility.HtmlEncode(_custBL.GetNameByCode(_temp.WrhsSubLed));
                //}
                //else if (_temp.WrhsFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Supplier))
                //{
                //    _warehouseSubled.Text = HttpUtility.HtmlEncode(_suppBL.GetSuppNameByCode(_temp.WrhsSubLed));
                //}
                //else
                //{
                //    _warehouseSubled.Text = "";
                //}

                Literal _supplier = (Literal)e.Item.FindControl("SupplierLiteral");
                _supplier.Text = HttpUtility.HtmlEncode(_temp.SuppName);

                Literal _poNo = (Literal)e.Item.FindControl("PONoLiteral");
                _poNo.Text = HttpUtility.HtmlEncode((_temp.PONo == "null") ? "" : _purchaseOrderBL.GetFileNmbrPRCPOHd(_temp.PONo, 0));
            }
        }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._receivingPOBL.GetSingleSTCReceiveHdApprove(_tempSplit);
            if (_ex == true)
            {
                String _result = this._receivingPOBL.DeleteMultiSTCReceiveHd(_tempSplit);
                if (_result == "")
                {
                ...
                else if (_result == "panel")
                {
                    this.ReasonPanel.Visible = true;
                }
                else
                {
                    Response.Redirect(this._homePage + "?" + ApplicationConfig.RequestPageKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_page, ApplicationConfig.EncryptionKey)) + "&" + ApplicationConfig.ActionResult + "=" + _result);
                }
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            //{
            //    MKTDOHd _var = _deliveryOrderBL.GetSingleMKTDOHd(_tempSplit[i]);
            //    if (_var.Status == DeliveryOrderDataMapper.GetStatus(DeliveryOrderStatus.Approved))
            //    {                    
            //        bool _result1 = _unpostingActivitiesBL.InsertUnpostingActivities(AppModule.GetValue(TransactionType.DeliveryOrder), _var.TransNmbr, _var.FileNmbr, HttpContext.Current.User.Identity.Name, this.ReasonTextBox.Text, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.UnPosting)));
            //    }
            //}

            bool _result = this._receivingPOBL.DeleteMultiApproveSTCReceiveHd(_tempSplit, AppModule.GetValue(TransactionType.ReceivingPO), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
