...
{
    public partial class CogsProcessDetail : CogsProcessBase
    {
        ...
        private string _reportPath1 = "StockAdjustment/STAdjustPrintPreview.rdlc";
        private string _reportPath2 = "ReceivingPO/JournalEntryPrintPreview.rdlc";
        private string _reportPath3 = "ReceivingPO/JournalEntryPrintPreviewHomeCurr.rdlc";
        ...
        private string _imgComplete = "complete2.jpg";
        ...
        private double RowCount()
        {
            ...
            _result = this._cogsProcessBL.RowsCountSTCProcessDt(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        {
            STCProcessHd _STCProcessHd = this._cogsProcessBL.GetStcProcessHdByCode(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
            ...
            this.StartTextBox.Text = DateFormMapper.GetValue(_STCProcessHd.StartDate);
            this.EndTextBox.Text = DateFormMapper.GetValue(_STCProcessHd.EndDate);
            ...
            {
                this.ListRepeater.DataSource = this._cogsProcessBL.GetListSTCProcessDt(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
            }
            this.ListRepeater.DataBind();

            //this.AllCheckBox.Checked = this.IsCheckedAll();

            if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower() ||
                this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Complete).ToString().ToLower()
                )
            {
                //this.AddButton.Visible = false;
                //this.DeleteButton.Visible = false;
            ...
            if (_nvcExtractor.GetValue(this._warning)!= "")
            {
                this.WarningLabel.Text = Rijndael.Decrypt(this._nvcExtractor.GetValue(this._warning), ApplicationConfig.EncryptionKey);
            }
        ...
        {
            if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                this.ActionButton.ImageUrl = ApplicationConfig.HomeWebAppURL + "images/" + _imgApprove;

                this._permGetApproval = this._permBL.PermissionValidation1(this._menuID, HttpContext.Current.User.Identity.Name, InetGlobalIndo.ERP.MTJ.Common.Enum.Action.Approve);
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                this.ActionButton.ImageUrl = ApplicationConfig.HomeWebAppURL + "images/" + _imgComplete;
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Complete).ToString().ToLower())
            {
        ...
        //        }
        //        else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
        //        {
        //            this.PreviewButton.Visible = true;
        //        }
        //        else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
        //        {
        //            this.PreviewButton.Visible = true;
        //        }
        //        else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
        ...
        {
            Response.Redirect(this._addPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)));
        }
        ...
        ////    bool _result = this._cogsProcessBL.DeleteMultiSTCAdjustDt(_tempSplit, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                STCProcessDt _temp = (STCProcessDt)e.Item.DataItem;
                ...
                //if (_STCProcessHd.Status != CogsProcessDataMapper.GetStatus(TransStatus.Posted))
                //{
                //    //_editButton.PostBackUrl = this._editPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)) + "&" + this._productKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code1, ApplicationConfig.EncryptionKey)) + "&" + this._locationKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code2, ApplicationConfig.EncryptionKey));
                ...
                Literal _location = (Literal)e.Item.FindControl("BeginQtyLiteral");
                _location.Text = HttpUtility.HtmlEncode((_temp.BeginQty == 0) ? "0" : _temp.BeginQty.ToString("#,##0.##"));

                Literal _qty = (Literal)e.Item.FindControl("OutQtyLiteral");
                _qty.Text = HttpUtility.HtmlEncode((_temp.OutQty == 0) ? "0" : _temp.OutQty.ToString("#,##0.##"));

                Literal _unit = (Literal)e.Item.FindControl("BalanceQtyLiteral");
                _unit.Text = HttpUtility.HtmlEncode((_temp.BalanceQty == 0) ? "0" : _temp.BalanceQty.ToString("#,##0.##"));

                Literal _adjust = (Literal)e.Item.FindControl("PriceCostLiteral");
                _adjust.Text = HttpUtility.HtmlEncode((_temp.PriceCost == 0) ? "0" : _temp.PriceCost.ToString("#,##0.##"));

                //if (_temp.FgAdjust.ToString() == "+")
                //{
                //    this.UpdateDetailPanel.Visible = true;
                //    Literal _price = (Literal)e.Item.FindControl("PriceLiteral");
                //    _price.Text = HttpUtility.HtmlEncode(_temp.PriceCost.ToString());
                //}
                //else
                //{
                //    this.UpdateDetailPanel.Visible = false;
                //}
        ...
        protected void ActionButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
                string _result = this._cogsProcessBL.GetAppr(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                string _result = this._cogsProcessBL.Approve(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);

                //string _result = this._cogsProcessBL.Posting(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);
            ...
            {
                string _result = this._cogsProcessBL.Complete(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == CogsProcessDataMapper.GetStatus(TransStatus.Complete).ToString().ToLower())
            {
                ...
                string _result = this._cogsProcessBL.Unposting(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);
                this.WarningLabel.Text = _result;
                // this.ReasonPanel.Visible = true;
                //string _result = this._stockAdjustBL.Unposting(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);

                //this.WarningLabel.Text = _result;
            }
            Response.Redirect(this._detailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(this.TransNoTextBox.Text, ApplicationConfig.EncryptionKey)) + "&" + this._warning + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(this.WarningLabel.Text, ApplicationConfig.EncryptionKey)));
            //this.ShowData(0);
        }
        ...
        //    ReportDataSource _reportDataSource1 = this._reportStockControlBL.STAdjustPrintPreview(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        //    _reportParam[0] = new ReportParameter("Nmbr", Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), true);
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            {
                bool _result1 = _unpostingActivitiesBL.InsertUnpostingActivities(AppModule.GetValue(TransactionType.StockAdjustment), this.TransNoTextBox.Text, this.FileNoTextBox.Text, HttpContext.Current.User.Identity.Name, this.ReasonTextBox.Text, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.UnPosting)));
                if (_result1 == true)
                    //this.WarningLabel.Text = _result;
                    Response.Redirect(this._editPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(this.TransNoTextBox.Text, ApplicationConfig.EncryptionKey) + "&" + this._warning + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_result, ApplicationConfig.EncryptionKey))));
