...
{
    public partial class StockMutation : ReportBase
    {
        ...
        private string _reportPath0 = "Report/StockMutation.rdlc";
        ...
        {
            this._permAccess = this._permBL.PermissionValidation1(this._menuIDStockMutation, HttpContext.Current.User.Identity.Name, InetGlobalIndo.ERP.MTJ.Common.Enum.Action.Access);
            ...
            if (!Page.IsPostBack == true)
            {
                ...
                this.PageTitleLiteral.Text = "Stock Mutation Report";
        ...
        protected void ShowWLocation()
        {
            this.WLocationCheckBoxList.ClearSelection();
            this.WLocationCheckBoxList.Items.Clear();
            this.WLocationCheckBoxList.DataSource = this._wrhsBL.GetListWrhsLocationForDDL();
            this.WLocationCheckBoxList.DataValueField = "WLocationCode";
            this.WLocationCheckBoxList.DataTextField = "WLocationName";
            this.WLocationCheckBoxList.DataBind();
        }
        ...
        private void ShowProduct(Int32 _prmCurrentPage)
        {
            ...
            this.ProductCodeCheckBoxList.DataSource = this._productBL.GetListDDLProductForReport(_tempProductSubGroup, _tempProductType, _prmCurrentPage, _maxrow);
        ...
        protected void ViewButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            var _hasilWLocation = this._wrhsBL.GetListWrhsLocationForDDL();

            for (var i = 0; i < _hasilWLocation.Count(); i++)
            {
                if (this.WLocationCheckBoxList.Items[i].Selected == true)
                {
                    if (_wLocationCode == "")
                    {
                        _wLocationCode += this.WLocationCheckBoxList.Items[i].Value;
                    }
                    else
                    {
                        _wLocationCode += "," + this.WLocationCheckBoxList.Items[i].Value;
            ...
            ReportDataSource _reportDataSource1 = this._report.StockMutation(_startDateTime, _endDateTime, _wrhsCode, _wLocationCode, _productCode);
            ...
            _reportParam[3] = new ReportParameter("WrhsCode", _wrhsCode, true);
            _reportParam[4] = new ReportParameter("ProductCode", _productCode, true);
            _reportParam[5] = new ReportParameter("LocationCode", _wLocationCode, false);
