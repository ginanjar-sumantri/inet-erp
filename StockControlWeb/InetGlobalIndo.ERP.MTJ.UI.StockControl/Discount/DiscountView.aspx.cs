...
namespace InetGlobalIndo.ERP.MTJ.UI.StockControl.Discount
{
    public partial class DiscountView : DiscountBase
    {
        ...
        public void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._discountBL.GetListDiscountLevel(_prmCurrentPage, _maxrow, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                Master_DiscountLevel _temp = (Master_DiscountLevel)e.Item.DataItem;
                string _discountCode = _temp.DiscountCode.ToString();
                string _levelCode = _temp.Level.ToString();
                string _discountLevelCode = _discountCode + "&" + _levelCode;

                if (this.TempHidden.Value == "")
                {
                    this.TempHidden.Value = _discountLevelCode;
                }
                else
                {
                    this.TempHidden.Value += "," + _discountLevelCode;
                ...
                _listCheckbox.Attributes.Add("OnClick", "CheckBox_Click(" + this.CheckHidden.ClientID + ", " + _listCheckbox.ClientID + ", '" + _discountLevelCode + "', '" + _awal + "', '" + _akhir + "', '" + _cbox + "')");
                _listCheckbox.Checked = this.IsChecked(_discountLevelCode);

                ImageButton _editButton = (ImageButton)e.Item.FindControl("EditButton");
                _editButton.PostBackUrl = this._editDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_discountCode, ApplicationConfig.EncryptionKey)) + "&" + _levelKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_levelCode, ApplicationConfig.EncryptionKey));
                ...
                Literal _level = (Literal)e.Item.FindControl("LevelLiteral");
                _level.Text = HttpUtility.HtmlEncode(_temp.Level.ToString());

                Literal _discount = (Literal)e.Item.FindControl("DiscountLiteral");
                _discount.Text = HttpUtility.HtmlEncode(_temp.Discount.ToString());
            }
        }
        ...
        private double RowCount()
        {
            ...
            _result = this._discountBL.RowsCountDiscountLevel(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _result = this._discountBL.DeleteMultiDiscountLevel(_tempSplit);
            ...
            Response.Redirect(this._viewPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)) + "&" + ApplicationConfig.RequestPageKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_page, ApplicationConfig.EncryptionKey)) + "&" + ApplicationConfig.ActionResult + "=" + _error);
        }
    }
}