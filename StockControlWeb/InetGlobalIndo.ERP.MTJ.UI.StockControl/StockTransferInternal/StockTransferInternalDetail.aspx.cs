...
{
    public partial class StockTransferInternalDetail : StockTransferInternalBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._stcTransInternalBL.RowsCountSTCTransferDt(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        {
            _stcTransInternalBL = new StockTransferInternalBL();
            ...
            this.WrhsAreaTextBox.Text = _wrhs.GetWrhsAreaNameByCode(_stcTransInternalHd.WrhsArea);
            this.WrhsSrcTextBox.Text = _wrhs.GetWarehouseNameByCode(_stcTransInternalHd.WrhsSrc);
            this.WrhsDestTextBox.Text = _wrhs.GetWarehouseNameByCode(_stcTransInternalHd.WrhsDest);
            this.OperatorTextBox.Text = _stcTransInternalHd.Operator;
            this.RemarkTextBox.Text = _stcTransInternalHd.Remark;
            if (_stcTransInternalHd.WrhsSrcFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.NoSubled))
            {
                this.SrcSubledTextBox.Text = "";
            }
            else
            {
                if (_stcTransInternalHd.WrhsSrcFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Customer))
                {
                    this.SrcSubledTextBox.Text = _custBL.GetNameByCode(_stcTransInternalHd.WrhsSrcSubLed);
                }
                else if (_stcTransInternalHd.WrhsSrcFgSubLed == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Supplier))
                {
                    this.SrcSubledTextBox.Text = _suppBL.GetSuppNameByCode(_stcTransInternalHd.WrhsSrcSubLed);
                }
            }
            if (_stcTransInternalHd.WrhsDestFgSubled == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.NoSubled))
            {
                this.DestSubledTextBox.Text = "";
            }
            else
            {
                if (_stcTransInternalHd.WrhsDestFgSubled == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Customer))
                {
                    this.DestSubledTextBox.Text = _custBL.GetNameByCode(_stcTransInternalHd.WrhsDestSubLed);
                }
                else if (_stcTransInternalHd.WrhsDestFgSubled == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Supplier))
                {
                    this.DestSubledTextBox.Text = _suppBL.GetSuppNameByCode(_stcTransInternalHd.WrhsDestSubLed);
                }
            }

            this.StatusHiddenField.Value = _stcTransInternalHd.Status.ToString();
            this.StatusLabel.Text = StockTransferInternalDataMapper.GetStatusText(_stcTransInternalHd.Status);
            ...
            {
                this.ListRepeater.DataSource = this._stcTransInternalBL.GetListSTCTransferDt(_prmCurrentPage, _maxrow, this.TransNoTextBox.Text);
            }
            this.ListRepeater.DataBind();

            this.AllCheckBox.Checked = this.IsCheckedAll();

            if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                this.AddButton.Visible = false;
                this.DeleteButton.Visible = false;
                this.EditButton.Visible = false;
        ...
        public void ShowPreviewButton()
        {
            ...
            {
                if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
                {
                    this.PreviewButton.Visible = false;
                }
                else if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
                {
                    this.PreviewButton.Visible = true;
                }
                else if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
                {
                    this.PreviewButton.Visible = true;
                }
                else if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
                {
                    this.PreviewButton.Visible = true;
                }
            }
        ...
        protected void ActionButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
                string _result = this._stcTransInternalBL.GetAppr(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                string _result = this._stcTransInternalBL.Approve(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
                string _result = this._stcTransInternalBL.Posting(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == StockTransferInternalDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                ...
                //string _result = this._stcTransInternalBL.Unposting(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _result = this._stcTransInternalBL.DeleteMultiSTCTransferDt(_tempSplit, this.TransNoTextBox.Text);
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                STCTransferDt _temp = (STCTransferDt)e.Item.DataItem;

                string _code1 = _temp.ProductCode.ToString();
                string _code2 = _temp.LocationSrc.ToString();
                string _code3 = _temp.LocationDest.ToString();
                string _all = _code1 + "|" + _code2 + "|" + _code3;
                ...
                _viewButton.PostBackUrl = this._viewDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(this.TransNoTextBox.Text, ApplicationConfig.EncryptionKey)) + "&" + this._productKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code1, ApplicationConfig.EncryptionKey)) + "&" + this._locationKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code2, ApplicationConfig.EncryptionKey)) + "&" + this._locationKey2 + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code3, ApplicationConfig.EncryptionKey));
                ...
                if (this.StatusHiddenField.Value.ToLower() != StockTransferInternalDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
                {
        ...
        protected void PreviewButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ReportDataSource _reportDataSource1 = this._reportStockControlBL.StockTransferInternalPrintPreview(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));

            this.ReportViewer1.LocalReport.DataSources.Clear();
            this.ReportViewer1.LocalReport.DataSources.Add(_reportDataSource1);

            string _path = _reportListBL.GetPathPrintPreview(AppModule.GetValue(TransactionType.StockTransferInternal), _userBL.GetCompanyId(HttpContext.Current.User.Identity.Name));
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            {
                bool _result1 = _unpostingActivitiesBL.InsertUnpostingActivities(AppModule.GetValue(TransactionType.StockTransferInternal), this.TransNoTextBox.Text, this.FileNoTextBox.Text, HttpContext.Current.User.Identity.Name, this.ReasonTextBox.Text, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.UnPosting)));
                if (_result1 == true)
                    this.WarningLabel.Text = _result;
