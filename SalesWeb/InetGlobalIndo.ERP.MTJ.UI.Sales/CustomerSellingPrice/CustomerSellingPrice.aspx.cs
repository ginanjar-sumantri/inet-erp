...
{
    public partial class CustomerSellingPrice : CustomerSellingPriceBase
    {
        private CustSellingPriceBL _custSellingPriceBL = new CustSellingPriceBL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.Page.IsPostBack == true)
            {
                ...
                this.btnSearchCustNo.OnClientClick = "window.open('../Search/Search.aspx?valueCatcher=findCustomer&configCode=customer','_popSearch','width=750,height=300,toolbar=0,location=0,status=0,scrollbars=1')";
                String spawnCust = "<script language='JavaScript'>\n";
                ////////////////////DECLARE FUNCTION FOR CATCHING SJ NO SEARCH
                spawnCust += "function findCustomer(x) {\n";
                spawnCust += "dataArray = x.split ('|') ;\n";
                spawnCust += "document.getElementById('" + this.CustomerTextBox.ClientID + "').value = dataArray [0];\n";
                spawnCust += "}\n";
        ...
        protected void ShowData(Int32 _prmCurrentPage)
        {
            ...
            this.ListRepeater.DataSource = this._custSellingPriceBL.getSellingPrice(_prmCurrentPage, _maxrow, this.CustomerTextBox.Text, this.ProductTextBox.Text, this.sortField.Value, Convert.ToBoolean(this.ascDesc.Value));
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                SALCustSellingPrice _salCustSellingPrice = (SALCustSellingPrice)e.Item.DataItem;
                ...
                _custCodeLiteral.Text = Convert.ToString(_salCustSellingPrice.CustCode);

                Literal _productCodeLiteral = (Literal)e.Item.FindControl("ProductCodeLiteral");
                _productCodeLiteral.Text = Convert.ToString(_salCustSellingPrice.ProductCode);

                Literal _transNmbrLiteral = (Literal)e.Item.FindControl("TransNmbrLiteral");
                _transNmbrLiteral.Text = Convert.ToString(_salCustSellingPrice.TransNmbr);

                Literal _transDateLiteral = (Literal)e.Item.FindControl("TransDateLiteral");
                _transDateLiteral.Text = DateFormMapper.GetValue(_salCustSellingPrice.TransDate);

                Literal _unitCodeLiteral = (Literal)e.Item.FindControl("UnitCodeLiteral");
                _unitCodeLiteral.Text = HttpUtility.HtmlEncode(_salCustSellingPrice.UnitCode);

                Literal _currCodeLiteral = (Literal)e.Item.FindControl("CurrCodeLiteral");
                _currCodeLiteral.Text = HttpUtility.HtmlEncode(_salCustSellingPrice.CurrCode);

                Literal _amoununtForexLiteral = (Literal)e.Item.FindControl("AmountForexLiteral");
                _amoununtForexLiteral.Text = HttpUtility.HtmlEncode(_salCustSellingPrice.AmountForex.ToString("#,##0.00"));
        ...
        {
            if (this.sortField.Value == "Customer Code")
            {
            ...
            {
                this.sortField.Value = "Customer Code";
                this.ascDesc.Value = "true";
            }
        ...
        private double RowCount()
        {
            ...
            _result = this._custSellingPriceBL.RowsCountSellingPrice(this.CustomerTextBox.Text , this.ProductTextBox.Text);
