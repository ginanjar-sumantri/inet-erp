...
namespace InetGlobalIndo.ERP.MTJ.UI.Sales.SalesOrder
{
    public partial class SalesOrder : SalesOrderBase
    {
        ...
        {
            if (this.sortField.Value == "Bill To")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
        ...
        private double RowCount()
        {
            ...
            _result = this._salesOrderBL.RowsCountMKTSOHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        //{
        //    double q = this._salesOrderBL.RowsCountMKTSOHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        //    if (min > 0)
        //    {
        //        sb.Append("<a href='" + this._homePage + "?" + ApplicationConfig.RequestPageKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt((i - 1).ToString(), ApplicationConfig.EncryptionKey)) + "'>" + ("<<< ") + "</a>&nbsp;");
        //    }
        ...
        //        else
        //        {
        //            sb.Append("<a href='" + this._homePage + "?" + ApplicationConfig.RequestPageKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(i.ToString(), ApplicationConfig.EncryptionKey)) + "'>" + (i + 1) + "</a>&nbsp;");
        //        }
        //    }

        //    if (max < (decimal)q)
        //    {
        //        sb.Append("<a href='" + this._homePage + "?" + ApplicationConfig.RequestPageKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(i.ToString(), ApplicationConfig.EncryptionKey)) + "'>" + (" >>>") + "</a>&nbsp;");
        //    }
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._salesOrderBL.GetListMKTSOHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, this.sortField.Value, Convert.ToBoolean(this.ascDesc.Value));
            }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._salesOrderBL.GetSingleMKTSOHdForStatus(_tempSplit);
            if (_ex == true)
            {
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                MKTSOHd _temp = (MKTSOHd)e.Item.DataItem;
                string _code = _temp.TransNmbr.ToString();
                string _code2 = _temp.Revisi.ToString();
                string _codeAll = _code + "-" + _code2;

                if (this.TempHidden.Value == "")
                {
                    this.TempHidden.Value = _codeAll;
                }
                else
                {
                    this.TempHidden.Value += "," + _codeAll;
                ...
                _listCheckbox.Attributes.Add("OnClick", "CheckBox_Click(" + this.CheckHidden.ClientID + ", " + _listCheckbox.ClientID + ", '" + _codeAll + "', '" + _awal + "', '" + _akhir + "', '" + _cbox + "')");
                _listCheckbox.Checked = this.IsChecked(_code);

                ImageButton _viewButton = (ImageButton)e.Item.FindControl("ViewButton");
                _viewButton.PostBackUrl = this._detailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey)) + "&" + this._codeRevisiKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code2, ApplicationConfig.EncryptionKey));
                ...
                {
                    if (_temp.Status != SalesOrderDataMapper.GetStatus(TransStatus.Posted))
                    {
                        _editButton.PostBackUrl = this._editPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey)) + "&" + this._codeRevisiKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code2, ApplicationConfig.EncryptionKey));
                ...
                //Literal _revisi = (Literal)e.Item.FindControl("RevisiLiteral");
                //_revisi.Text = HttpUtility.HtmlEncode(_temp.Revisi.ToString());

                Literal _status = (Literal)e.Item.FindControl("StatusLiteral");
                _status.Text = SalesOrderDataMapper.GetStatusText(_temp.Status);

                Literal _currency = (Literal)e.Item.FindControl("CurrLiteral");
                _currency.Text = HttpUtility.HtmlEncode(_temp.CurrCode);

                Literal _custName = (Literal)e.Item.FindControl("CustLiteral");
                _custName.Text = HttpUtility.HtmlEncode(_custBL.GetNameByCode(_temp.CustCode));

                Literal _termName = (Literal)e.Item.FindControl("TermLiteral");
                _termName.Text = HttpUtility.HtmlEncode(_termBL.GetTermNameByCode(_temp.Term));

                Literal _billToName = (Literal)e.Item.FindControl("BillToLiteral");
                _billToName.Text = HttpUtility.HtmlEncode(_custBL.GetNameByCode(_temp.BillTo));
            }
        }
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._salesOrderBL.DeleteMultiApproveMKTSOHd(_tempSplit, AppModule.GetValue(TransactionType.SalesOrder), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
