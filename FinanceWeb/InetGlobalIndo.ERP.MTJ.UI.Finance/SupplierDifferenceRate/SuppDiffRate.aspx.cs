...
{
    public partial class SuppDiffRate : SupplierDifferenceRateBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._finAPRateBL.RowsCountFINAPRateHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._finAPRateBL.GetListFINAPRateHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
            }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._finAPRateBL.GetSingleFINAPRateHdForStatus(_tempSplit);
            if (_ex == true)
            {
                bool _result = this._finAPRateBL.DeleteMultiFINAPRateHd(_tempSplit);
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                FINAPRateHd _temp = (FINAPRateHd)e.Item.DataItem;
                ...
                {
                    if (_temp.Status != APRateDataMapper.GetStatus(TransStatus.Posted))
                    {
                ...
                _status.Text = HttpUtility.HtmlEncode(APRateDataMapper.GetStatusText(_temp.Status));

                Literal _currency = (Literal)e.Item.FindControl("CurrLiteral");
                _currency.Text = HttpUtility.HtmlEncode(_temp.CurrCode);

                Literal _newRate = (Literal)e.Item.FindControl("NewRateLiteral");
                byte _decimalPlace = this._currencyBL.GetDecimalPlace(_temp.CurrCode);
                _newRate.Text = HttpUtility.HtmlEncode((_temp.NewRate == 0) ? "0" : _temp.NewRate.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace)));
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._finAPRateBL.DeleteMultiApproveFINAPRateHd(_tempSplit, AppModule.GetValue(TransactionType.APRate), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
