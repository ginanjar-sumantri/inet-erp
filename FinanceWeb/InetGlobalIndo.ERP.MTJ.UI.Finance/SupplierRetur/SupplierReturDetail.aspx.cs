...
{
    public partial class SupplierReturDetail : SupplierReturBase
    {
        ...
        {
            _supplierReturBL = new SupplierReturBL();
            FINSuppReturHd _suppReturHd = this._supplierReturBL.GetSingleFINSuppReturHd(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));

            this._decimalPlace = _currencyBL.GetDecimalPlace(_suppReturHd.CurrCode);

            this.TransNoTextBox.Text = _suppReturHd.TransNmbr;
            this.FileNmbrTextBox.Text = _suppReturHd.FileNmbr;
            this.DateTextBox.Text = DateFormMapper.GetValue(_suppReturHd.TransDate);
            this.StatusLabel.Text = CustomerNoteDataMapper.GetStatusText(_suppReturHd.Status);
            this.SuppTextBox.Text = this._supplierBL.GetSuppNameByCode(_suppReturHd.SuppCode);
            this.SuppCodeHiddenField.Value = _suppReturHd.SuppCode;
            this.CurrCodeTextBox.Text = _suppReturHd.CurrCode;
            this.CurrRateTextBox.Text = (_suppReturHd.ForexRate == 0) ? "0" : _suppReturHd.ForexRate.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            this.TermTextBox.Text = _suppReturHd.Term;
            this.CurrTextBox.Text = _suppReturHd.CurrCode;
            this.StatusLabel.Text = CustomerNoteDataMapper.GetStatusText(_suppReturHd.Status);
            this.StatusHiddenField.Value = _suppReturHd.Status.ToString();
            this.AttnTextBox.Text = _suppReturHd.Attn;
            this.RemarkTextBox.Text = _suppReturHd.Remark;
            if (_suppReturHd.PPNRate != null)
            {
                decimal _ppnRate = Convert.ToDecimal(_suppReturHd.PPNRate);
                this.PPNRateTextBox.Text = (_ppnRate == 0) ? "0" : _ppnRate.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            }
            if (_suppReturHd.PPN != null)
            {
                decimal _ppn = Convert.ToDecimal(_suppReturHd.PPN);
                this.PPNPercentTextBox.Text = (_ppn == 0) ? "0" : _ppn.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            }
            this.PPNNoTextBox.Text = _suppReturHd.PPNNo;
            if (_suppReturHd.PPNForex != null)
            {
                decimal _ppnForex = Convert.ToDecimal(_suppReturHd.PPNForex);
                this.PPNForexTextBox.Text = (_ppnForex == 0) ? "0" : _ppnForex.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            }
            if (_suppReturHd.PPNDate != null)
            {
                this.PPNDateTextBox.Text = DateFormMapper.GetValue((DateTime)_suppReturHd.PPNDate);
            }
            if (_suppReturHd.BaseForex != null)
            {
                decimal _amountBase = Convert.ToDecimal(_suppReturHd.BaseForex);
                this.AmountBaseTextBox.Text = (_amountBase == 0) ? "0" : _amountBase.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            }

            if (_suppReturHd.TotalForex != null)
            {
                decimal _totalForex = Convert.ToDecimal(_suppReturHd.TotalForex);
                this.TotalForexTextBox.Text = (_totalForex == 0) ? "0" : _totalForex.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            }

            //this.BillToTextBox.Text = this._supplierBL.GetNameByCode(_suppReturHd.BillTo);
        ...
        public void ShowDataDetail(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._supplierReturBL.GetlistFINSuppReturDt(_prmCurrentPage, _maxrow, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
            }
            this.ListRepeater.DataBind();

            List<FINSuppReturDt> _finSuppReturDt = this._supplierReturBL.GetlistFINSuppReturDt(_prmCurrentPage, _maxrow, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));

            if (_finSuppReturDt.Count != 0)
            {
                this.AddButton.Visible = false;
            }
            else
        ...
        protected void ActionButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == SupplierInvoiceDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
                string _result = this._supplierReturBL.GetAppr(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

                this.Label1.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == SupplierInvoiceDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                string _result = this._supplierReturBL.Approve(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);

                this.Label1.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == SupplierInvoiceDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
                string _result = this._supplierReturBL.Posting(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);
        ...
        {
            Response.Redirect(this._addDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey))+"&"+ this._suppCode + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(this.SuppCodeHiddenField.Value, ApplicationConfig.EncryptionKey)));
            //Response.Redirect(this._addDetailPage);
        }

        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            List<FINSuppReturDt> _finSuppReturDt = this._supplierReturBL.GetlistFINSuppReturDt(0, _maxrow, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
            ...
            if (_finSuppReturDt.Count != 0)
            {
                ...
                bool _result = this._supplierReturBL.DeleteMultiSTCReturRRDt(_tempSplit, this.TransNoTextBox.Text);
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                FINSuppReturDt _temp = (FINSuppReturDt)e.Item.DataItem;
                FINSuppReturHd _finCustReturHd = this._supplierReturBL.GetSingleFINSuppReturHd(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
                ...
                String _allCode = _temp.TransNmbr.ToString() + "|" + _temp.SJNo.ToString() + "|" + _temp.ProductCode.ToString();
                ...
                _account.Text = HttpUtility.HtmlEncode(_temp.SJNo);
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            string _result = this._supplierReturBL.Unposting(this.TransNoTextBox.Text, _year, _period, HttpContext.Current.User.Identity.Name);
