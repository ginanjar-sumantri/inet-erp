...
{
    public partial class MoneyChanger : MoneyChangerBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._moneyChangerBL.RowsCountMoneyChanger(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._moneyChangerBL.GetListMoneyChanger(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                Finance_MoneyChanger _temp = (Finance_MoneyChanger)e.Item.DataItem;

                string _code = _temp.MoneyChangerCode.ToString();
                ...
                {
                    if (_temp.Status.ToString().Trim().ToLower() == MoneyChangerDataMapper.GetStatusByte(TransStatus.Posted).ToString().Trim().ToLower())
                    {
                        _editButton.Visible = false;
                    }
                    else
                ...
                _type.Text = MoneyChangerDataMapper.GetTypeText(MoneyChangerDataMapper.GetType(_temp.FgType));

                Literal _typeExchange = (Literal)e.Item.FindControl("TypeExchangeLiteral");
                _typeExchange.Text = MoneyChangerDataMapper.GetTypeText(MoneyChangerDataMapper.GetType(_temp.FgTypeExchange));

                Literal _status = (Literal)e.Item.FindControl("StatusLiteral");
                _status.Text = MoneyChangerDataMapper.GetStatusText(_temp.Status);

                Literal _currency = (Literal)e.Item.FindControl("CurrencyLiteral");
                _currency.Text = HttpUtility.HtmlEncode(_temp.CurrCode);

                Literal _currExchange = (Literal)e.Item.FindControl("CurrExchangeLiteral");
                _currExchange.Text = HttpUtility.HtmlEncode(_temp.CurrExchange);
                ...
                _amount.Text = HttpUtility.HtmlEncode((_temp.Amount).ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace)));

                byte _decimalPlaceAmount = this._currencyBL.GetDecimalPlace(_temp.CurrExchange);
                Literal _amountExchange = (Literal)e.Item.FindControl("AmountExchangeLiteral");
                _amountExchange.Text = HttpUtility.HtmlEncode((_temp.AmountExchange).ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlaceAmount)));
            }
        }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._moneyChangerBL.GetSingleMoneyChangerForStatus(_tempSplit);
            if (_ex == true)
            {
                bool _result = this._moneyChangerBL.DeleteMultiMoneyChanger(_tempSplit);
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._moneyChangerBL.DeleteMultiApproveMoneyChanger(_tempSplit, AppModule.GetValue(TransactionType.MoneyChanger), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
