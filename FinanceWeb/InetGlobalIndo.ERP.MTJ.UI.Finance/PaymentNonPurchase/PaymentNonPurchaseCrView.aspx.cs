...
{
    public partial class PaymentNonPurchaseCrView : PaymentNonPurchaseBase
    {
        ...
        {
            FINPayNonTradeCr _finPayNonTradeCr = this._paymentNonPurchaseBL.GetViewFINPayNonTradeCr(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeItemKey), ApplicationConfig.EncryptionKey));
            ...
            this.PaymentTypeTextBox.Text = _finPayNonTradeCr.PayName;
            this.DocumentNoTextBox.Text = _finPayNonTradeCr.DocumentNo;
            this.NominalTextBox.Text = _finPayNonTradeCr.AmountForex.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            this.BankPaymentTextBox.Text = _finPayNonTradeCr.BankName;
            this.DateTextBox.Text = DateFormMapper.GetValue(_finPayNonTradeCr.DueDate);
            decimal _bankExpense = Convert.ToDecimal((_finPayNonTradeCr.BankExpense == null) ? 0 : _finPayNonTradeCr.BankExpense);
            this.BankChargesTextBox.Text = (_bankExpense == 0) ? "" : _bankExpense.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
