...
namespace Windows_App
{
    class dbConnector
    {
        ...
        public void insertSMSGatewayReceive(MessagesBlock _prmMessage)
        {
            ...
            String _dateSent = _prmMessage.Sent;
            _dateSent = _dateSent.Replace('\"', ' ').Trim();
            DateTime _tes = Convert.ToDateTime(_dateSent);
            _dateSent = _tes.ToString("dd/yy/MM hh:mm:ss tt");
            String _category = "";
            ...
            if (!cekExistingData("SMSGatewayReceive", "Message", "'" + _prmMessage.Message + "'"))
                ...
                executeNonQuery("INSERT INTO SMSGatewayReceive ( Category, SenderPhoneNo, Message, flagRead,userID, dateReceived,OrganizationID ) VALUES ('" + _category + "','" + (_senderPhoneNo.Substring(1, _senderPhoneNo.Length - 2)) + "','" + _message + "',0,'" + GlobalVar.UserID + "', '" + _dateSent + "'," + GlobalVar.OrgID + ")");
        }
