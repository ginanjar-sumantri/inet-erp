...
public class Captcha : IHttpHandler, IReadOnlySessionState
{
    public void ProcessRequest(HttpContext context) {
        ...
        Bitmap bmpOut = new Bitmap(160, 30);

        Graphics g = Graphics.FromImage(bmpOut);

        g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
        g.FillRectangle(Brushes.Blue, 0, 0, 160, 30);
        ...
        for (int i = 0; i < 88; i++)
            g.FillRectangle(_randomColor[ran.Next(0, 19)], ran.Next(0, 80), ran.Next(0, 15), ran.Next(0, 80), ran.Next(0, 15));
        for (int i = 0; i < 8; i++)
            g.FillRectangle(Brushes.White, ran.Next(0, 50), ran.Next(0, 25), ran.Next(0, 100), ran.Next(0, 3));
