...
namespace SMS.SMSWeb.Statistic
{
    public partial class Statistic_Statistic : StatisticBase
    {
        ...
        protected void ShowData() {
            this.TodaySentSMSLiteral.Text = _statisticBL.GetTodaySentSMS(_organization, _userID).ToString();
            this.TodayIncomingSMSLiteral.Text = _statisticBL.GetTodayIncomingSMS(_organization, _userID).ToString();
            this.TodayJunkLiteral.Text = _statisticBL.GetTodayJunkSMS(_organization, _userID).ToString();

            this.ThisMonthSentSMSLiteral.Text = _statisticBL.GetThisMonthSentSMS(_organization, _userID).ToString();
            this.ThisMonthIncomingSMSLiteral.Text = _statisticBL.GetThisMonthIncomingSMS(_organization, _userID).ToString();

            this.BalanceIncomingLiteral.Text = _statisticBL.GetIncomingBalanceTotal(_organization).ToString("#,##0.00");

            this.SentSMSByCategoryRepeater.DataSource = _statisticBL.GetListSentSMSByCategory(this.CategoryMonthDropDownList.SelectedValue, this.CategoryYearDropDownList.SelectedValue, _organization);
            this.SentSMSByCategoryRepeater.DataBind();

            if (Convert.ToBoolean(_fgAdmin))
            {
                this.SentSMSPerUserRepeater.DataSource = _statisticBL.GetListSentSMSByUser(this.UserMonthDropDownList.SelectedValue, this.UserYearDropDownList.SelectedValue, _organization);
                this.SentSMSPerUserRepeater.DataBind();

                this.BalanceIncomingRepeater.DataSource = _statisticBL.GetListTransactionIncoming(_organization);
                this.BalanceIncomingRepeater.DataBind();

                this.BalanceOutGoingRepeater.DataSource = _statisticBL.GetListOutgoingMasking(_organization, this.OutBalanceYearDropDownList.SelectedValue);
