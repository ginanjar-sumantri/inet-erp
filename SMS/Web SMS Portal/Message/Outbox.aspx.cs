...
{
    public partial class Outbox : MessageBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._smsMessageBL.RowsCountSMSGatewaySendOutbox(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, this.CategoryDDL.SelectedValue, this.StatusDropDownList.SelectedValue, _UserID, _organization, _fgAdmin);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            this.ListRepeater.DataSource = this._smsMessageBL.GetListSMSGatewaySendOutbox(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, this.CategoryDDL.SelectedValue, this.StatusDropDownList.SelectedValue, _UserID, _organization, _fgAdmin);
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                SMSGatewaySend _smsSend = (SMSGatewaySend)e.Item.DataItem;
                string _code = _smsSend.id.ToString();
                ...
                _viewButton.PostBackUrl = this._outboxViewPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                ...
                _catLiteral.Text = HttpUtility.HtmlEncode(_smsSend.Category);

                Literal _destinationLiteral = (Literal)e.Item.FindControl("DestinationLiteral");
                _destinationLiteral.Text = HttpUtility.HtmlEncode(_smsSend.DestinationPhoneNo.ToString());

                Literal _statusLiteral = (Literal)e.Item.FindControl("StatusLiteral");
                _statusLiteral.Text = HttpUtility.HtmlEncode((_smsSend.flagSend == null || _smsSend.flagSend == 0) ? "Queued" : "Failed");

                Literal _userNameLiteral = (Literal)e.Item.FindControl("UserNameLiteral");
                _userNameLiteral.Text = HttpUtility.HtmlEncode(_smsSend.userID);

                Literal _dateSentLiteral = (Literal)e.Item.FindControl("DateSentLiteral");
                _dateSentLiteral.Text = HttpUtility.HtmlEncode(DateFormMapper.GetValue(_smsSend.DateSent));
            }
        }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            Response.Redirect(this._outboxPage + "?" + ApplicationConfig.RequestPageKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_page, ApplicationConfig.EncryptionKey)) + "&" + ApplicationConfig.ActionResult + "=" + _error);
        }
