...
function ChangeFormat2(_prmTextBox, _prmDecimalPlace) {
    _prmTextBox.value = FormatCurrency2(_prmTextBox.value, _prmDecimalPlace.valueOf());
}
...
function FormatCurrency2(_prmCurrency, _prmDecimalPlace) {
    ...
    _cents = LeftPad(_cents, String(_prmDecimalPlace).length - 1, "0");
...
function AskToDeleteScheduleShift() {
    var _result = false;

    if (confirm("Are you sure want to delete schedule shift in this day?") == true) {
        _result = true;
    }
    else {
        _result = false;
