...
{
    public partial class POSInternet : POSInternetBase
    {
        protected MemberForBL _memberBL = new MemberForBL();
        protected POSTrInternetBookingBL _internetBookingBL = new POSTrInternetBookingBL();
        protected InternetFloorBL _interentFloorBL = new InternetFloorBL();
        ...
        protected void RenderLayout()
        {
            try
            {
                ...
                {
                    int _tableStatus = this._posInternetBL.GetTableStatus(this.currFloorHiddenField.Value, i);
                    if (_splitedPosition.Length < i || this.hiddenBoxPosition.Value == "")
                    {
                        ...
                        {
                            _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox available\"" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + ">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                        }
                        if (_tableStatus == POSTrInternetDataMapper.GetStatusTable(POSMsInternetTableStatus.Booking))
                        {
                            _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox reserved\"" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + ">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                        }
                        if (_tableStatus == POSTrInternetDataMapper.GetStatusTable(POSMsInternetTableStatus.NotAvailable))
                        {
                            if (this._histTableBL.Is15MinsLeft(this.currFloorHiddenField.Value, i))
                            {
                                _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox not-available15min\"" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + ">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                            }
                            else
                            {
                                _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox not-available\"" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + ">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (_tableStatus == POSTrInternetDataMapper.GetStatusTable(POSMsInternetTableStatus.Available))
                        {
                            _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox available\" style=\"top:" + (Convert.ToInt16(_splitedXY[1]) + _yAdjust).ToString() + "px;left:" + (Convert.ToInt16(_splitedXY[0]) + _xAdjust).ToString() + "px;" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + "\">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                        }
                        if (_tableStatus == POSTrInternetDataMapper.GetStatusTable(POSMsInternetTableStatus.Booking))
                        {
                            _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox reserved\" style=\"top:" + (Convert.ToInt16(_splitedXY[1]) + _yAdjust).ToString() + "px;left:" + (Convert.ToInt16(_splitedXY[0]) + _xAdjust).ToString() + "px;" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + "\">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                        }
                        if (_tableStatus == POSTrInternetDataMapper.GetStatusTable(POSMsInternetTableStatus.NotAvailable))
                        {
                            if (this._histTableBL.Is15MinsLeft(this.currFloorHiddenField.Value, i))
                            {
                                _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox not-available15min\" style=\"top:" + (Convert.ToInt16(_splitedXY[1]) + _yAdjust).ToString() + "px;left:" + (Convert.ToInt16(_splitedXY[0]) + _xAdjust).ToString() + "px;" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + "\">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                            }
                            else
                            {
                                _strDivBoxes += "<div id=\"divBox" + i.ToString() + "\" class=\"divBox not-available\" style=\"top:" + (Convert.ToInt16(_splitedXY[1]) + _yAdjust).ToString() + "px;left:" + (Convert.ToInt16(_splitedXY[0]) + _xAdjust).ToString() + "px;" + ((_tableStatus > 0) ? _backgroundStyleJunction : "") + "\">" + this._posInternetBL.GetTableNumber(this.currFloorHiddenField.Value, i) + "</div>";
                            }
                        }
        ...
        {
            //int _tableStatus = this._posInternetBL.GetTableStatus(this.currFloorHiddenField.Value,Convert.ToInt32(this.SelectedTableHiddenField.Value));
        ...
        protected void FillFromToTable()
        {
            try
            {
                List<POSMsInternetTable> _listOccupiedTable = this._posInternetBL.GetListOccupiedTable(this.currFloorHiddenField.Value);
                ...
                this.ToTableDropDownList.DataSource = this._posInternetBL.GetListAvailableTable(this.currFloorHiddenField.Value);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Page.IsPostBack)
                {
                    this._nvcExtractor = new NameValueCollectionExtractor(Request.QueryString);
                    this.currFloorHiddenField.Value = Rijndael.Decrypt(this._nvcExtractor.GetValue(this._selectedFloorKey), ApplicationConfig.EncryptionKey);
                    POSMsInternetFloor _pOSMsInternetFloor = _interentFloorBL.GetSinglePOSMsInternetFloorByRoomCode(Convert.ToString(this.currFloorHiddenField.Value));
                    this.currFloorNameHiddenField.Value = _pOSMsInternetFloor.FloorName;

                    this.CheckStatusImageButton.OnClientClick = "window.open('../General/CheckStatus.aspx?pos=internet','_popSearch','fullscreen=yes,toolbar=0,location=0,status=0,scrollbars=1')";
                    this.TableTransferImageButton.Enabled = false;
                    //this._internetTableBL.UpdateTables();

                    this.FloorButtonRepeater.DataSource = this._posInternetBL.GetList(Convert.ToInt16(this.floorPagingPageHiddenField.Value), 2);
        ...
        protected void DownFloorPanelButton_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                if (Convert.ToInt16(this.floorPagingPageHiddenField.Value) < this._posInternetBL.GetLastPageFloorButton(2))
                {
        ...
        {
            Response.Redirect("POSInternetReservation.aspx" + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt("", ApplicationConfig.EncryptionKey)));
        }
        ...
        protected void TableTransferImageButton_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                int _newID = _tableHistBL.GetNewID();
                Boolean _result = false;
                _result = this._posInternetBL.TableTransfer(this.FromTableDropDownList.SelectedValue, this.ToTableDropDownList.SelectedValue, this.currFloorNameHiddenField.Value, _newID);

                if (_result == true)
                {
                    this.RefreshPage();
                    this.RefreshPage2();
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "MsgBox", "alert('Transfer Table Successfully');", true);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "MsgBox", "alert('Transfer Table Failed');", true);
                }
        ...
        private void ShowData()
        {
            try
            {
                this.BookingListRepeater.DataSource = this._internetBookingBL.GetList();
        ...
        protected void BookingListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
                if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    string _code = _temp.InternetBookCode.ToString(); //+ "-" + _temp.CustomerID.ToString() + "-" + _temp.CircuitID.ToString();
                    string _code2 = _temp.InternetBookCode.ToString() + "-" + _temp.ItemNo.ToString();
                    ...
                    _openButton.CommandArgument = _temp.HistoryID.ToString() + "-" + _temp.InternetBookCode.ToString() + "-" + _temp.ItemNo.ToString();

                    Literal _noMemberLiteral = (Literal)e.Item.FindControl("NoMemberLiteral");
                    _noMemberLiteral.Text = HttpUtility.HtmlEncode(this._internetBookingBL.GetMemberCode(_temp.InternetBookCode));

                    Literal _nameLiteral = (Literal)e.Item.FindControl("NameLiteral");
                    _nameLiteral.Text = HttpUtility.HtmlEncode(this._internetBookingBL.GetCustName(_temp.InternetBookCode));

                    Literal _noTelponLiteral = (Literal)e.Item.FindControl("NoTelponLiteral");
                    _noTelponLiteral.Text = HttpUtility.HtmlEncode(this._internetBookingBL.GetPhoneNmbr(_temp.InternetBookCode));

                    Literal _tableLiteral = (Literal)e.Item.FindControl("TableLiteral");
                    _tableLiteral.Text = HttpUtility.HtmlEncode(Convert.ToString(this._interentFloorBL.GetMemberNameByCodeInternet(Convert.ToString(_temp.FloorNmbr)) + "/" + _temp.TableIDPerRoom));
                    ...
                    _operatorNameLiteral.Text = HttpUtility.HtmlEncode(this._internetBookingBL.GetOperatorName(_temp.InternetBookCode));
                }
            }
            catch (Exception ex)
            {
        ...
        protected void BookingListRepeater_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "OpenBooked")
                {
                    ...
                    POSTrInternetBookingDt _StatusInternetBooking = this._internetBookingBL.GetSingleForStatus(_code);
                    _StatusInternetBooking.FgActive = false;
                    _StatusInternetBooking.HistoryID = Convert.ToInt32(_maxHistoryID + 1);
                    this._internetBookingBL.Edit(_StatusInternetBooking);

                    POSTableStatusHistory _statusHistory = this._histTableBL.GetSingleForInternet(_code);
        ...
        protected void ButtonStopInternet_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                String _result = this._posInternetBL.StopInternet(this.currFloorNameHiddenField.Value, this.TableDropDownList.SelectedValue);
                if (_result == "")
                {
                    ...
                    this.RefreshPage();
                    this.RefreshPage2();
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "MsgBox", "alert('Stop Time Successfully');", true);
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "MsgBox", "alert('Stop Time Failed');", true);
        ...
        protected void ButtonAddTimeInternet_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                String _result = this._posInternetBL.AddTimeInternet(this.currFloorNameHiddenField.Value, this.TableDropDownList.SelectedValue, Convert.ToInt32(this.TimeDropdownList.SelectedValue));
                if (_result == "")
                {
                    //this.WarningTypeHiddenField.Value = "Success";
                    //this.WarningHiddenField.Value = "Add Time Success";
                    this.RefreshPage();
                    //ScriptManager.RegisterStartupScript(this, this.GetType(), "Msgbox", "javascript:$.msg({autoUnblock: false,content: 'Added Time Successfully'});", true);
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "MsgBox", "alert('Added Time Successfully');", true);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "MsgBox", "alert('Added Time Fail');", true);
                    //ScriptManager.RegisterStartupScript(this, this.GetType(), "Msgbox", "javascript:$.msg({autoUnblock: false,content: 'Added Time Fail'});", true);
                    //    this.WarningTypeHiddenField.Value = "Error";
                    //    this.WarningHiddenField.Value = _result;
                }
            }
        ...
        private void RefreshPage()
        {
            try
            {
                this._internetTableBL.UpdateTables("Internet");
                this.ComposeLayout(this.currFloorHiddenField.Value);
                this.ReadPosition(this.currFloorHiddenField.Value);
                this.RenderLayout();
                //this.FillFromToTable();
        ...
        protected void TableInfoImageButton_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                {
                    POSTrInternetHd _pOSTrInternetHd = this._posInternetBL.GetSinglePOSTableStatusHistory(this._posInternetBL.GetFloorNmbrByName(this.currFloorHiddenField.Value), Convert.ToInt16(this.TableDropDownList.SelectedValue));
                    if (_pOSTrInternetHd != null)
                    {
                        this.ReferenceLiteral.Text = _pOSTrInternetHd.ReferenceNo;
                        this.CustNameLiteral.Text = _pOSTrInternetHd.CustName;
                    }
                    Int32 _id = this._histTableBL.GetID(this._posInternetBL.GetFloorNmbrByName(this.currFloorHiddenField.Value), Convert.ToInt32(this.TableDropDownList.SelectedValue), POSFloorTypeDataMapper.GetFloorType(POSFloorType.Internet));
                    POSTableStatusHistory _posTableStatusHistory = this._histTableBL.GetSingleForInternet(_id.ToString());
