...
namespace InetGlobalIndo.ERP.MTJ.UI.Purchasing.PurchaseRetur
{
    public partial class PurchaseRetur : PurchaseReturBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._purchaseReturBL.RowsCountPRCReturHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._purchaseReturBL.GetListPRCReturHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, this.sortField.Value, Convert.ToBoolean(this.ascDesc.Value));
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                PRCReturHd _temp = (PRCReturHd)e.Item.DataItem;
                ...
                {
                    if (_temp.Status != PurchaseReturDataMapper.GetStatusByte(TransStatus.Posted))
                    {
                ...
                _status.Text = HttpUtility.HtmlEncode(PurchaseReturDataMapper.GetStatusText(_temp.Status));

                //Literal _warehouse = (Literal)e.Item.FindControl("WarehouseLiteral");
                //_warehouse.Text = HttpUtility.HtmlEncode(_warehouseBL.GetWarehouseNameByCode(_receivePOBL.GetWarehouseByCode(_temp.RRNo)));

                //Literal _warehouseSubled = (Literal)e.Item.FindControl("WrhsSubledLiteral");
                //char _fgSubled = _receivePOBL.GetWarehouseFgSubledByCode(_temp.RRNo);
                //if (_fgSubled == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Customer))
                //{
                //    _warehouseSubled.Text = HttpUtility.HtmlEncode(_custBL.GetNameByCode(_receivePOBL.GetWarehouseSubledByCode(_temp.RRNo)));
                //}
                //else if (_fgSubled == WarehouseDataMapper.GetSubledStatus(WarehouseSubledStatus.Supplier))
                //{
                //    _warehouseSubled.Text = HttpUtility.HtmlEncode(_suppBL.GetSuppNameByCode(_receivePOBL.GetWarehouseSubledByCode(_temp.RRNo)));
                //}
                //else
                //{
                ...
                _rrNo.Text = HttpUtility.HtmlEncode(_receivePOBL.GetFileNmbrSTCReceiveHd(_temp.RRNo));
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._purchaseReturBL.GetSinglePRCReturHdApprove(_tempSplit);
            if (_ex == true)
            {
                bool _result = this._purchaseReturBL.DeleteMultiPRCReturHd(_tempSplit);
                if (_result == true)
                {
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._purchaseReturBL.DeleteMultiApprovePRCReturHd(_tempSplit, AppModule.GetValue(TransactionType.PurchaseRetur), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
