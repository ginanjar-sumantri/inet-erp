...
{
    public partial class BILTrRadiusActivateTemp : CustBillingAccountBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._custBillAccountBL.RowsCountBILTrRadiusActiveTemp(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._custBillAccountBL.GetListBILTrRadiusActiveTemp(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                BILTrRadiusActivateTemporary _temp = (BILTrRadiusActivateTemporary)e.Item.DataItem;
                ...
                _viewButton.PostBackUrl = this._viewPageTrRadiusTemporary + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                ...
                if (_temp.Status != BilTrRadiusActivateTempDataMapper.GetStatus(TransStatus.Posted))
                {
                    _editButton.ImageUrl = ApplicationConfig.HomeWebAppURL + "images/edit.jpg";
                    _editButton.PostBackUrl = this._editPageTrRadiusTemporary + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                }
                else
                {
                ...
                Literal _custBillNametLiteral = (Literal)e.Item.FindControl("CustomerNameLiteral");
                //string _custCode = this._custBillAccountBL.GetCustCodeByCustbillcode(_temp.CustBillCode);
                _custBillNametLiteral.Text = HttpUtility.HtmlEncode(this._custBillAccountBL.GetCustNameByCustCode(_temp.CustCode));

                Literal _custBillAccountLiteral = (Literal)e.Item.FindControl("FileNotLiteral");
                _custBillAccountLiteral.Text = HttpUtility.HtmlEncode(_temp.FileNmbr);

                Literal _custLiteral = (Literal)e.Item.FindControl("TransNoLiteral");
                _custLiteral.Text = HttpUtility.HtmlEncode(_temp.TransNmbr);

                Literal _productLiteral = (Literal)e.Item.FindControl("TransdateLiteral");
                _productLiteral.Text = HttpUtility.HtmlEncode(DateFormMapper.GetValue(_temp.Transdate));

                if (_temp.Status != null)
                {
                    Literal _descLiteral = (Literal)e.Item.FindControl("StatusLiteral");
                    _descLiteral.Text = HttpUtility.HtmlEncode(BilTrRadiusActivateTempDataMapper.GetStatusText((char)_temp.Status));
                }
                ...
                _periodeLiteral.Text = HttpUtility.HtmlEncode(_temp.Period.ToString());
            }
        }
        ...
        protected void ListRepeater_OnItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            //    bool _result = this._custBillAccountBL.EditBILTrRadiusActivate(_bilTrRadiusActivate);
        ...
        {
            Response.Redirect(this._addPageTrRadiusTemporary);
        }
