...
{
    public partial class FAService : FAServiceBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._faServiceBL.RowsCountFAServiceHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._faServiceBL.GetListFAServiceHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                GLFAServiceHd _glFAServiceHd = (GLFAServiceHd)e.Item.DataItem;

                string _code = _glFAServiceHd.TransNmbr.ToString();
                ...
                {
                    if (_glFAServiceHd.Status != FAServiceDataMapper.GetStatus(TransStatus.Posted))
                    {
                ...
                _transNoLiteral.Text = HttpUtility.HtmlEncode(_glFAServiceHd.TransNmbr);

                Literal _fileNmbrLiteral = (Literal)e.Item.FindControl("FileNmbrLiteral");
                _fileNmbrLiteral.Text = HttpUtility.HtmlEncode(_glFAServiceHd.FileNmbr);

                Literal _transDateLiteral = (Literal)e.Item.FindControl("TransDateLiteral");
                _transDateLiteral.Text = HttpUtility.HtmlEncode(DateFormMapper.GetValue(_glFAServiceHd.TransDate));

                Literal _statusLiteral = (Literal)e.Item.FindControl("StatusLiteral");
                _statusLiteral.Text = FAServiceDataMapper.GetStatusText(_glFAServiceHd.Status);

                Literal _faCodeLiteral = (Literal)e.Item.FindControl("FACodeLiteral");
                _faCodeLiteral.Text = HttpUtility.HtmlEncode(_glFAServiceHd.FACode);

                Literal _faNameLiteral = (Literal)e.Item.FindControl("FANameLiteral");
                _faNameLiteral.Text = HttpUtility.HtmlEncode(_glFAServiceHd.FAName);
            }
        }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._faServiceBL.GetSingleFAServiceHdApprove(_tempSplit);
            if (_ex == true)
            {
                bool _result = this._faServiceBL.DeleteMultiFAServiceHd(_tempSplit);
                if (_result == true)
                {
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._faServiceBL.DeleteMultiApproveFAServiceHd(_tempSplit, AppModule.GetValue(TransactionType.FAService), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
