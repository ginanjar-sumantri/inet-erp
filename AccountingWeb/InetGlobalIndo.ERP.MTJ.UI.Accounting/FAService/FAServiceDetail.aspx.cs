...
{
    public partial class FAServiceDetail : FAServiceBase
    {
        ...
        private string _reportPath1 = "FAService/FAServicePrintPreview.rdlc";
        private string _reportPath2 = "FAService/JournalEntryPrintPreview.rdlc";
        private string _reportPath3 = "FAService/JournalEntryPrintPreviewHomeCurr.rdlc";
        ...
        {
            if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
        ...
        public void ShowCreateJurnalButton()
        {
            ...
            else
            {
                ...
                if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
                {
        ...
        private void ShowData()
        {
            ...
            _decimalPlace = _currencyBL.GetDecimalPlace(_glFAServiceHd.CurrCode);
            ...
            this.SupplierTextBox.Text = this._supplierBL.GetSuppNameByCode(_glFAServiceHd.SuppCode);
            this.AttnTextBox.Text = _glFAServiceHd.Attn;
            this.CurrencyTextBox.Text = _glFAServiceHd.CurrCode;
            this.ForexRateTextBox.Text = (_glFAServiceHd.ForexRate == 0) ? "0" : _glFAServiceHd.ForexRate.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            this.TermTextBox.Text = this._termBL.GetTermNameByCode(_glFAServiceHd.Term);
            ...
            this.FixedAssetTextBox.Text = this._fixedAssetBL.GetFixedAssetNameByCode(_glFAServiceHd.FACode);
            this.PeriodTextBox.Text = _periodBL.GetDescByPeriod(_glFAServiceHd.StartPeriod);
            this.YearTextBox.Text = _glFAServiceHd.StartYear.ToString();
            ...
            this.StatusLabel.Text = FAServiceDataMapper.GetStatusText(_glFAServiceHd.Status);
            this.StatusHiddenField.Value = _glFAServiceHd.Status.ToString();
        ...
        private double RowCount()
        {
            ...
            _result = this._faServiceBL.RowsCountFAServiceDt(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        private void ShowDetail(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._faServiceBL.GetListFAServiceDt(_prmCurrentPage, this._maxrow, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
            }
            ...
            if (this._nvcExtractor.GetValue(ApplicationConfig.ActionResult) != "")
            {
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                GLFAServiceDt _temp = (GLFAServiceDt)e.Item.DataItem;
                ...
                _viewButton.PostBackUrl = this._viewDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)) + "&" + this._codeItemKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                ...
                else
                {
                    ...
                    {
                        _editButton.PostBackUrl = this._editDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)) + "&" + this._codeItemKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                ...
                Literal _faMaintenanceCodeLiteral = (Literal)e.Item.FindControl("FAMaintenanceCodeLiteral");
                _faMaintenanceCodeLiteral.Text = HttpUtility.HtmlEncode(_temp.FAMaintenance);

                Literal _faMaintenanceNameLiteral = (Literal)e.Item.FindControl("FAMaintenanceNameLiteral");
                _faMaintenanceNameLiteral.Text = HttpUtility.HtmlEncode(_temp.FAMaintenanceName);

                Literal _addValueLiteral = (Literal)e.Item.FindControl("AddValueLiteral");
                _addValueLiteral.Text = _temp.FgAddValue.ToString();

                Literal _remarkLiteral = (Literal)e.Item.FindControl("RemarkLiteral");
                _remarkLiteral.Text = HttpUtility.HtmlEncode(_temp.Remark);

                decimal _tempQty = (_temp.Qty == null) ? 0 : Convert.ToDecimal(_temp.Qty);
                Literal _qtyLiteral = (Literal)e.Item.FindControl("QtyLiteral");
                _qtyLiteral.Text = (_tempQty == 0) ? "0" : _tempQty.ToString("#,###.##");

                Literal _unitLiteral = (Literal)e.Item.FindControl("UnitLiteral");
                _unitLiteral.Text = HttpUtility.HtmlEncode(_temp.Unit);

                decimal _tempPriceForex = (_temp.PriceForex == null) ? 0 : Convert.ToDecimal(_temp.PriceForex);
                Literal _priceForexLiteral = (Literal)e.Item.FindControl("PriceForexLiteral");
                _priceForexLiteral.Text = (_tempPriceForex == 0) ? "0" : _tempPriceForex.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));

                decimal _tempAmountForex = (_temp.AmountForex == null) ? 0 : Convert.ToDecimal(_temp.AmountForex);
                Literal _amountForexLiteral = (Literal)e.Item.FindControl("AmountForexLiteral");
                _amountForexLiteral.Text = (_tempAmountForex == 0) ? "0" : _tempAmountForex.ToString(CurrencyDataMapper.GetFormatDecimal(_decimalPlace));
            }
        }
        ...
        protected void PreviewButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ReportDataSource _reportDataSource1 = this._reportBL.FAServicePrintPreview(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        protected void ActionButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
                string _result = this._faServiceBL.GetApprove(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);

                this.Label.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                string _result = this._faServiceBL.Approve(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);

                this.Label.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
                string _result = this._faServiceBL.Posting(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAServiceDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                ...
                //string _result = this._faServiceBL.Unposting(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), _year, _period, HttpContext.Current.User.Identity.Name);
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _result = this._faServiceBL.DeleteMultiFAServiceDt(_tempsplit, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            {
                bool _result1 = _unpostingActivitiesBL.InsertUnpostingActivities(AppModule.GetValue(TransactionType.FAService), this.TransNoTextBox.Text, this.FileNmbrTextBox.Text, HttpContext.Current.User.Identity.Name, this.ReasonTextBox.Text, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.UnPosting)));
                if (_result1 == true)
