...
{
    public partial class FASales : FASalesBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._faSalesBL.RowsCountFASalesHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._faSalesBL.GetListFASalesHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                GLFASalesHd _glFASalesHd = (GLFASalesHd)e.Item.DataItem;

                string _code = _glFASalesHd.TransNmbr.ToString();
                ...
                _transNoLiteral.Text = HttpUtility.HtmlEncode(_glFASalesHd.TransNmbr);

                Literal _fileNmbrLiteral = (Literal)e.Item.FindControl("FileNmbrLiteral");
                _fileNmbrLiteral.Text = HttpUtility.HtmlEncode(_glFASalesHd.FileNmbr);

                Literal _transDateLiteral = (Literal)e.Item.FindControl("TransDateLiteral");
                _transDateLiteral.Text = HttpUtility.HtmlEncode(DateFormMapper.GetValue(_glFASalesHd.TransDate));

                Literal _statusLiteral = (Literal)e.Item.FindControl("StatusLiteral");
                _statusLiteral.Text = HttpUtility.HtmlEncode(Convert.ToString(_glFASalesHd.Status));

                Literal _custNameLiteral = (Literal)e.Item.FindControl("CustNameLiteral");
                _custNameLiteral.Text = HttpUtility.HtmlEncode(_glFASalesHd.CustName);

                Literal _currCodeLiteral = (Literal)e.Item.FindControl("CurrCodeLiteral");
                _currCodeLiteral.Text = HttpUtility.HtmlEncode(_glFASalesHd.CurrCode);
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._faSalesBL.GetSingleGLFASalesHdApprove(_tempSplit);
            if (_ex == true)
            {
                bool _result = this._faSalesBL.DeleteMultiFASalesHd(_tempSplit);
                if (_result == true)
                {
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._faSalesBL.DeleteMultiApproveFASalesHd(_tempSplit, AppModule.GetValue(TransactionType.FASales), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
