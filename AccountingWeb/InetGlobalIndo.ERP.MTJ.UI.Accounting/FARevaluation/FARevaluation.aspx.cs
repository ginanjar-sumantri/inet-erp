...
{
    public partial class FARevaluation : FARevaluationBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._faRevalutionBL.RowsCountGLFADevaluationHd(this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._faRevalutionBL.GetListGLFADevaluationHd(_prmCurrentPage, _maxrow, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text, this.sortField.Value, Convert.ToBoolean(this.ascDesc.Value));
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                GLFADevaluationHd _temp = (GLFADevaluationHd)e.Item.DataItem;
                ...
                {
                    if (_temp.Status != FixedAssetStatus.GetStatus(TransStatus.Posted))
                    {
                ...
                _status.Text = HttpUtility.HtmlEncode(FixedAssetStatus.GetStatusText(_temp.Status));

                Literal _remark = (Literal)e.Item.FindControl("RemarkLiteral");
                _remark.Text = HttpUtility.HtmlEncode(_temp.Remark);

                Literal _operator = (Literal)e.Item.FindControl("OperatorLiteral");
                _operator.Text = HttpUtility.HtmlEncode(_temp.Operator);
            }
        }
        ...
        protected void DeleteButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            bool _ex = this._faRevalutionBL.GetSingleGLFADevaluationHdApprove(_tempSplit);
            if (_ex == true)
            {
                bool _result = this._faRevalutionBL.DeleteMultiGLDevaluationHd(_tempSplit);
                if (_result == true)
                {
        ...
        {
            if (this.sortField.Value == "Operator")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            bool _result = this._faRevalutionBL.DeleteMultiApproveGLDevaluationHd(_tempSplit, AppModule.GetValue(TransactionType.FADevaluation), HttpContext.Current.User.Identity.Name, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.Deleted)), this.ReasonTextBox.Text);
            if (_result == true)
            {
