...
{
    public partial class FAAddStockDetail : FAAddStockBase
    {
        ...
        private string _reportPath1 = "FAAddStock/GLFAAddStockPrintPreview.rdlc";
        private string _reportPath2 = "FAAddStock/JournalEntryPrintPreview.rdlc";
        private string _reportPath3 = "FAAddStock/JournalEntryPrintPreviewHomeCurr.rdlc";
        ...
        private double RowCount()
        {
            ...
            _result = this._faAddStockBL.RowsCountFAAddStockDt(Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey));
        ...
        {
            _faAddStockBL = new StockChangeToFABL();
            ...
            this.StatusLabel.Text = FAAddStockDataMapper.GetStatusText(_glFAAddStockHd.Status);
            this.StatusHiddenField.Value = _glFAAddStockHd.Status.ToString();

            if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                this.EditButton.Visible = false;
            }
            else
        ...
        {
            if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
        ...
        public void ShowDetail(Int32 _prmCurrentPage)
        {
            ...
            {
                this.ListRepeater.DataSource = this._faAddStockBL.GetListFAAddStockDt(_prmCurrentPage, _maxrow, Rijndael.Decrypt(this._nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), this.sortField.Value, Convert.ToBoolean(this.ascDesc.Value));
            }
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                GLFAAddStockDt _temp = (GLFAAddStockDt)e.Item.DataItem;

                string _code = _temp.GLFAAddStockDtCode.ToString();
                ...
                _viewButton.PostBackUrl = this._viewDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)) + "&" + this._code + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                ...
                else
                {
                    ...
                    {
                        _editButton.PostBackUrl = this._editDetailPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(this._nvcExtractor.GetValue(this._codeKey)) + "&" + this._code + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_code, ApplicationConfig.EncryptionKey));
                ...
                _productLiteral.Text = _temp.ProductName;

                Literal _locationLiteral = (Literal)e.Item.FindControl("LocationLiteral");
                _locationLiteral.Text = _temp.LocationName;

                Literal _faStatusLiteral = (Literal)e.Item.FindControl("FAStatusLiteral");
                _faStatusLiteral.Text = _temp.FAStatusName;

                Literal _faOwnerLiteral = (Literal)e.Item.FindControl("FAOwnerLiteral");
                _faOwnerLiteral.Text = HttpUtility.HtmlEncode(FAAddStockDataMapper.GetStatusText(_temp.FAOwner));

                Literal _faSubGroupLiteral = (Literal)e.Item.FindControl("FASubGroupLiteral");
                _faSubGroupLiteral.Text = _temp.FASubGroupName;

                Literal _faNameLiteral = (Literal)e.Item.FindControl("FANameLiteral");
                _faNameLiteral.Text = _temp.FAName;
            }
        }
        ...
        protected void ActionButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.OnHold).ToString().ToLower())
            {
                string _result = this._faAddStockBL.GetApproval(this.TransNoTextBox.Text, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.WaitingForApproval).ToString().ToLower())
            {
                string _result = this._faAddStockBL.Approve(this.TransNoTextBox.Text, HttpContext.Current.User.Identity.Name);

                this.WarningLabel.Text = _result;
            }
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.Approved).ToString().ToLower())
            {
                string _result = this._faAddStockBL.Posting(this.TransNoTextBox.Text, HttpContext.Current.User.Identity.Name);
            ...
            else if (this.StatusHiddenField.Value.Trim().ToLower() == FAAddStockDataMapper.GetStatus(TransStatus.Posted).ToString().ToLower())
            {
                ...
                //string _result = this._faAddStockBL.UnPosting(this.TransNoTextBox.Text, HttpContext.Current.User.Identity.Name);
        ...
        protected void PreviewButton_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ReportDataSource _reportDataSource1 = this._reportBL.GLFAAddStockPrintPreview(this.TransNoTextBox.Text);
        ...
        {
            if (this.sortField.Value == "Product")
            {
            ...
            {
                this.sortField.Value = "Product";
                this.ascDesc.Value = "true";
            }
            this.ShowDetail(Convert.ToInt32(this.ViewState[this._currPageKey]));
        }
        protected void field2_Click(object sender, EventArgs e)
        {
            if (this.sortField.Value == "Location")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
        ...
        {
            if (this.sortField.Value == "FA Name")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
        ...
        {
            if (this.sortField.Value == "FA Condition")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
            ...
            {
                this.sortField.Value = "FA Condition";
                this.ascDesc.Value = "true";
            }
            this.ShowDetail(Convert.ToInt32(this.ViewState[this._currPageKey]));
        }
        ...
        {
            if (this.sortField.Value == "FA Owner")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
        ...
        {
            if (this.sortField.Value == "FA SubGroup")
            {
                if (this.ascDesc.Value == "" || this.ascDesc.Value == "false")
                    this.ascDesc.Value = "true";
                else
            ...
            {
                this.sortField.Value = "FA SubGroup";
                this.ascDesc.Value = "true";
            }
            this.ShowDetail(Convert.ToInt32(this.ViewState[this._currPageKey]));
        }
        ...
        protected void YesButton_OnClick(object sender, EventArgs e)
        {
            ...
            {
                bool _result1 = _unpostingActivitiesBL.InsertUnpostingActivities(AppModule.GetValue(TransactionType.FAAddStock), this.TransNoTextBox.Text, this.FileNmbrTextBox.Text, HttpContext.Current.User.Identity.Name, this.ReasonTextBox.Text, Convert.ToByte(ActivityTypeDataMapper.GetActivityType(ActivityType.UnPosting)));
                if (_result1 == true)
                    this.WarningLabel.Text = _result;
