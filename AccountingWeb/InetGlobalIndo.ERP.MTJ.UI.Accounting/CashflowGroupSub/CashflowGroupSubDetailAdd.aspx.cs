...
{
    public partial class CashflowGroupSubDetailAdd : CashflowGroupSubBase
    {
        ...
        private double RowCount()
        {
            ...
            _result = this._cashFlowGroupBL.RowsCountNotAssign(Rijndael.Decrypt(_nvcExtractor.GetValue(this._codeKey), ApplicationConfig.EncryptionKey), this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private void ShowData(Int32 _prmCurrentPage)
        {
            ...
            this.LabelCode.Text = _finMsCFgroupSub.CashFlowGroupSubCode;
            this.LabelName.Text = _finMsCFgroupSub.CashFlowGroupSubName;

            this.ListRepeater.DataSource = _cashFlowGroupBL.GetListNotAssign(_prmCurrentPage, _maxrow, _finMsCFgroupSub.CashFlowGroupSubCode, this.CategoryDropDownList.SelectedValue, this.KeywordTextBox.Text);
        ...
        private Boolean IsAccountSelected(string _prmAccountCode)
        {
            Boolean _result = false;

            string[] _accountCode = this.CheckHidden.Value.Split(',');

            for (int i = 0; i < _accountCode.Length; i++)
            {
                if (_accountCode[i] == _prmAccountCode)
                {
                    _result = true;
        ...
        protected void ListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                ...
                _cb.Checked = this.IsAccountSelected(_code);
                //_cb.Attributes.Add("OnClick", "CheckBox_Click(" + this.CheckHidden.ClientID + "," + _cb.ClientID + ",'" + _code + "', '" + awal + "' , '" + akhir + "', '" + cbox + "','" + _tempHidden + "')");
                _cb.Attributes.Add("OnClick", "CheckBox_Click(" + this.CheckHidden.ClientID + "," + _cb.ClientID + ",'" + _code + "', '" + awal + "' , '" + akhir + "', '" + cbox + "' );");
        ...
        {
            bool _result = false;
            if (this.AssignAllCheckbox.Checked == true)
            {
                _result = _cashFlowGroupBL.AddSubGroupAccount(this.LabelCode.Text);
            }
            else
            {
                string _temp = this.CheckHidden.Value;
                _result = this._cashFlowGroupBL.AddSelectedAccount(_temp, LabelCode.Text);
            }
        }
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect(_transViewPage + "?" + this._codeKey + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(this.LabelCode.Text, ApplicationConfig.EncryptionKey)) + "&" + this._codeKey2 + "=" + HttpUtility.UrlEncode(Rijndael.Encrypt(_prmKey2, ApplicationConfig.EncryptionKey)));
        }
